...
Launch a Metal blockchain validator node in minutes's decentralized cloud for a fraction of traditional VPS costs. Metal blockchain, developed by Metallicus, powers a compliant Digital Asset Banking Network connecting traditional finance with blockchain technology for regulated institutions.
...
Mainnet: Approximately $5-15/month (varies by provider and market conditions)
...
✅ IP configured: 184.105.162.180:9650
   (Verify this matches your IP Console → Leases → IP(s))
...
1. **Copy and securely save** the base64 backup string that appears
2. Store it somewhere safe (encrypted password manager, secure note, etc.)
...
Complete the two-step deployment process. Set `METAL_PUBLIC_IP` to your LoadBalancer IP Console → Update deployment.
