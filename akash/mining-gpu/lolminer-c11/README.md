...
* Added support for LHR unlock in 470.103.01 and all 510.x drivers. In fact _we highly recommend_ using these drivers, because it will give a more stable unlocker experience!
...
* Added optimizes solvers for Beam Hash III for AMD & Nvidia cards. Use _--coin BEAM_ to auto switch from BeamHash II to BeamHash III on fork (approx June 28th, requires 8G card) or select Beam Hash III solver manually with _--algo BEAM-III_ (requires 6G card)
* Added performance improved (+ >10%) GRIN-C29M solver for 8G GPUs
* Added Cuckaroo-30 solver to mine Cortex Ai (_--coin CTXC_ or _--algo C30CTX_) for all 8G and higher GPUs
